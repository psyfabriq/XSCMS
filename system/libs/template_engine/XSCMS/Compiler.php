...
class Compiler
{
    ...
    public static function tagInclude(Tokenizer $tokens, Tag $tag)
    {
        ...
        if ($name) {
            if ($tpl->getStorage()->getOptions() & \XSCMS::FORCE_INCLUDE) {
    ...
    {
        trigger_error("XSCMS: tag {for} deprecated, use {foreach 1..4 as \$value} (in {$scope->tpl->getName()}:{$scope->line})", E_USER_DEPRECATED);
        $p = array(
            "index" => false,
            "first" => false,
            "last"  => false,
...
//            "min"   => false
    ...
    public static function stdFuncOpen(Tokenizer $tokens, Tag $tag)
    {
        ...
        $tag->setOption(\XSCMS::AUTO_ESCAPE, false);
        return 'ob_start();';
    }
    ...
    public static function escapeOpen(Tokenizer $tokens, Tag $tag)
    {
        ...
        $tokens->next();
        $tag->setOption(\XSCMS::AUTO_ESCAPE, $expected);
    }
    ...
    public static function stripOpen(Tokenizer $tokens, Tag $tag)
    {
        $expected = ($tokens->get(T_STRING) == "true" ? true : false);
        $tokens->next();
        $tag->setOption(\XSCMS::AUTO_STRIP, $expected);
    }
