...
use XSCMS;
use XSCMS\Error\UnexpectedTokenException;
use XSCMS\Error\CompileException;
use XSCMS\Error\InvalidUsageException;
use XSCMS\Error\SecurityException;
use XSCMS\Error\TokenizeException;
...
class Template extends Render
{
    ...
    /**
     * @param XSCMS $fenom Template storage
     * @param int $options
     * @return \XSCMS\Template
     */
    public function __construct(XSCMS $fenom, $options)
    {
        $this->_fenom       = $fenom;
    ...
    private function _appendText($text)
    {
        ...
        if($this->_options & XSCMS::AUTO_STRIP) {
...
//            $text = preg_replace('/\s*([\pP\pS]+)\s*/uS', '$1', $text);
    ...
    public function getTemplateCode()
    {
        ...
        return "<?php \n" .
        "/** XSCMS template '" . $this->_name . "' compiled at " . date('Y-m-d H:i:s') . " */\n" .
        $before . // some code 'before' template
        "return new XSCMS\\Render(\$fenom, " . $this->_getClosureSource() . ", array(\n" .
    ...
    {
        if ($escape === null) {
            $escape = $this->_options & XSCMS::AUTO_ESCAPE;
        }
        if ($escape) {
    ...
    public function parseTerm(Tokenizer $tokens, &$is_var = false, $allows = -1)
    {
        ...
        if ($tokens->is(T_LNUMBER, T_DNUMBER)) {
            ...
            if ($tokens->is("(") && $tokens->hasBackList(T_STRING, T_OBJECT_OPERATOR)) {
                if ($this->_options & XSCMS::DENY_METHODS) {
                    throw new \LogicException("Forbidden to call methods");
                }
                $code = $unary . $this->parseChain($tokens, $code);
            } elseif ($tokens->is(Tokenizer::MACRO_INCDEC)) {
                if($this->_options & XSCMS::FORCE_VERIFY) {
    ...
    {
        if ($this->_options & XSCMS::DENY_STATICS) {
