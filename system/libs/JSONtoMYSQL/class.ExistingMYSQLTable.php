...
class ExistingMYSQLTable extends AbstractMysqlTable{
	...
	public function __construct($mysql, $tablename, $primary="id", $parentid="pid"){
	...
	public function save($json_obj){
		...
		if(is_array($json_obj) && isset($json_obj[$parentid])){
			$pid_value = $json_obj[$parentid];
		}else if(is_object($json_obj) && isset($json_obj->$parentid)){
			$pid_value = $json_obj->$parentid;
		}
...
/* 				echo "need to handle object subdata\n"; */
		...
		if(strlen($set)){
			...
			return $result = $this->mysql->query($sql);
		}
	}
